...
def test_generate_repo_name_raises_on_empty_arg(team_name, master_repo_name,
                                                empty_arg):
...
def directory_structure(tmpdir):
    ...
    for dir_, filenames in [(tmpdir, root_files), (repo, repo_files),
...
class TestGenerateReviewAllocations:
    ...
    def test_throws_when_too_many_reviews(self, num_students, num_reviews):
        ...
        with pytest.raises(ValueError) as exc_info:
            util.generate_review_allocations(master_repo_name, students,
                                             num_reviews)
    ...
    def test_throws_when_too_few_reviews(self):
        with pytest.raises(ValueError) as exc_info:
            util.generate_review_allocations('week-2',
                                             list(string.ascii_lowercase), 0)
        assert 'num_reviews must be greater than 0' in str(exc_info)

    @pytest.mark.parametrize('num_students, num_reviews', [(10, 4), (50, 13),
                                                           (10, 1)])
    def test_all_students_allocated_same_amount_of_times(
        ...
        allocations = util.generate_review_allocations('week-2', students,
                                                       num_reviews)
    ...
    @pytest.mark.parametrize('num_students, num_reviews', [(10, 4), (50, 3),
                                                           (10, 1)])
    def test_all_students_get_reviewed(self, num_students, num_reviews):
        ...
        allocations = util.generate_review_allocations(master_repo_name,
