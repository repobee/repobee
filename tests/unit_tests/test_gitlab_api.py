...
def api_mock(mocker):
    mocker.patch(
        "repobee.ext.gitlab_api.gitlab.Gitlab", side_effect=GitLabMock
    )
...
class TestEnsureTeamsAndMembers:
    def test_with_no_pre_existing_groups(self, api_mock):
        ...
        """
        # arrange
        api = repobee.ext.gitlab_api.GitLabAPI(BASE_URL, TOKEN, TARGET_GROUP)
...
class TestCreateRepos:
    ...
    def api(self, api_mock):
        yield repobee.ext.gitlab_api.GitLabAPI(BASE_URL, TOKEN, TARGET_GROUP)
